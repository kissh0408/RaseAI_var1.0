"""track_bias_daily: fold2 5シードモデルのOOSスコアを書き出し、v39ベースラインと比較する。"""
...
from train_fold2 import FEATURES_PATH, BIAS_PATH, MODELS_DIR, FOLD  # noqa: E402

EXP_DIR = Path(__file__).resolve().parent
SCORES_PATH = EXP_DIR / "scores" / "scores_track_bias_fold2_oos.parquet"
...
def main() -> None:
    ...
    if len(model_paths) != 5:
        raise ValueError(f"モデル数 {len(model_paths)}（期待5）: {MODELS_DIR}")
    ...
    print(f"{'指標':12s} {'baseline(v39)':>15s} {'track_bias':>15s} {'差分':>10s}")
