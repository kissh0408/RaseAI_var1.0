"""market_leak_diagnostic: fold2(市場情報つき)スコアで L2/L3 OOS 測定を走らせる。

prob_fusion/src/run_fit_oos.py と betting/src/run_backtest_oos.py は
evaluation/reports/{fusion_oos_fold2.json, betting_backtest_oos.json} を
ハードコードで上書きするため、本番レポートを退避 → 実行 → 実験用ディレクトリへ
コピー → 本番レポートを復元、という手順で汚染を防ぐ。
"""
...
SCORES_PATH = EXP_DIR / "scores" / "scores_market_leak_fold2_oos.parquet"
...
def main() -> None:
    if not SCORES_PATH.is_file():
        raise FileNotFoundError(f"{SCORES_PATH} がありません。先に export_scores.py を実行してください。")
    ...
    try:
        ...
        shutil.copy2(FUSION_REPORT, EXP_DIR / "reports" / "fusion_oos_fold2_market_leak.json")

        run_backtest_oos(SCORES_PATH, FEATURES_PATH, FUSION_REPORT)
        shutil.copy2(BETTING_REPORT, EXP_DIR / "reports" / "betting_backtest_oos_market_leak.json")
    finally:
    ...
    print(f"\n実験結果:")
    print(f"  {EXP_DIR / 'reports' / 'fusion_oos_fold2_market_leak.json'}")
    print(f"  {EXP_DIR / 'reports' / 'betting_backtest_oos_market_leak.json'}")
