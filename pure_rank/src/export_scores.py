...
def export_scores(split: str = "test") -> Path:
    ...
    df = df.copy()
    df["ensemble_score"] = ensemble_predict(models, df[feat_cols])

    out_cols = [
        "race_id", "race_date", "ketto_num", "horse_num",
        "course_code", "finish_rank", "ensemble_score",
    ]
    out_cols = [c for c in out_cols if c in df.columns]
    out_df = df[out_cols].sort_values(["race_date", "race_id", "horse_num"])

    out_dir = feat_dir / "exported_scores"
    out_dir.mkdir(parents=True, exist_ok=True)
    out_path = out_dir / f"scores_{version}_{split}.parquet"
