...
def _collect_bets_per_race(
    ...
    Parameters
    ----------
    prob_method : "harville"（デフォルト・既存動作）または "stern"
                  "stern" の場合 lam2, lam3 の指定が必須。bracket_models は
                  Harville 確率分布でフィットされているため stern では適用しない
                  （呼び出し側で bracket_models=None にすること）。
    """
    if prob_method not in ("harville", "stern"):
        raise ValueError(f"prob_method must be 'harville' or 'stern', got: {prob_method!r}")
    if prob_method == "stern" and (lam2 is None or lam3 is None):
        raise ValueError("prob_method='stern' requires lam2 and lam3 to be provided")
